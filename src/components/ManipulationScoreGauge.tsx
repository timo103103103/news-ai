...
const categorizeFactorsToDimensions = (factors: { name: string; score: number }[]): BiasDimension[] => {
  // We simulate mapping backend factors to Professional Analyst Dimensions
  // In a real scenario, specific backend keys would map to these specific axes
  ...
  // If specific factors aren't found, we distribute the average risk across dimensions 
  // to ensure the chart is populated for the demo, weighted by the "nature" of the dimension.
...
export default function ManipulationScoreGauge({ 
  ...
  if (!report) return <div className="p-6 text-gray-500 italic">Initializing Analysis Engine...</div>;

  const biasColor = getBiasColor(report.biasLevel);

  // Data for the Donut Chart (Objectivity)
  const pieData = [
      ...
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden font-sans">
        ...
        <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Editorial Objectivity Analysis</h3>
              <p className="text-xs text-slate-500">Automated Linguistic & Rhetorical Review</p>
            </div>
          </div>
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-slate-200 shadow-sm">
            ...
            <span className="text-xs font-bold text-slate-700">{report.biasLevel} Classification</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
          ...
          <div className="lg:col-span-4 p-6 border-r border-slate-100 flex flex-col justify-between">
            ...
            <div className="relative mb-6">
              <div className="h-48 w-full relative z-10">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pt-8">
                  <span className="text-4xl font-extrabold text-slate-800">{report.objectivityScore}<span className="text-lg text-slate-400">/100</span></span>
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-widest mt-1">Objectivity Index</span>
                </div>
              </div>
            </div>
          ...
          {/* --- CENTER COLUMN: Radar Analysis (5 cols) --- */}
          <div className="lg:col-span-5 p-6 bg-slate-50/50">
            ...
            <div className="h-64 w-full relative">
              ...
              <div className="absolute top-0 right-0 p-2 bg-white/80 backdrop-blur rounded text-[10px] text-slate-500 border border-slate-100 shadow-sm max-w-[120px]">
                *Graph expands outward based on intensity of detected bias factors.
              </div>
            </div>

            {/* Analyst Verdict Box */}
            <div className="mt-2 p-4 bg-white border border-slate-200 rounded-lg shadow-sm relative">
              <div className="absolute top-0 left-4 -translate-y-1/2 bg-slate-800 text-white text-[10px] px-2 py-0.5 rounded uppercase font-bold tracking-wider">
                Analyst Verdict
              </div>
          ...
          {/* --- RIGHT COLUMN: Evidence & Fingerprints (3 cols) --- */}
          <div className="lg:col-span-3 p-6 border-l border-slate-100 bg-slate-50">
             <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
            ...
            {report.linguisticTriggers.length > 0 ? (
              <div className="space-y-3">
                ...
                {report.linguisticTriggers.slice(0, 5).map((trigger, idx) => (
                  <div key={idx} className="bg-white px-3 py-2 rounded border border-slate-200 shadow-sm flex items-start gap-2">
              ...
              <div className="flex flex-col items-center justify-center h-40 text-center">
                ...
                <p className="text-sm font-medium text-slate-700">Clean Pattern</p>
                <p className="text-xs text-slate-500">No significant bias markers detected.</p>
              </div>
            )}
            ...
            <div className="mt-8 pt-6 border-t border-slate-200">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-600 mb-1">
                ...
                <span>Primary Goal: <span className="font-semibold text-slate-800">{report.biasLevel === 'Neutral' ? 'Inform' : 'Persuade'}</span></span>
              </div>
              <div className="flex items-center gap-2 text-xs text-slate-600">
                <Activity className="w-3 h-3 text-purple-500" />
                <span>Tone: <span className="font-semibold text-slate-800">{report.editorialTone}</span></span>
              </div>
            </div>
          </div>
